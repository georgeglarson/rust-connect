...
// existing connection-layer self-guards (landed in `e5b8058`, 2026-07-29):
...
// Control for the three refuse_* tests above (task-0.3-review.md
...
// task-0.3-review.md Important-1: `load_from_disk` (called from
...
/// vk #992: a pairing that times out must not leave its staged peer cert
...
async fn test_expired_request_drops_staged_cert() {
    ...
    assert!(
        !cert_manager.has_peer_fingerprint(&device_id),
        "the timed-out pairing's staged cert must not leak into a later accept (vk #992)"
    );
}

/// vk #992 (cli-review-panel follow-up): `cleanup_expired` is the OTHER
...
async fn test_cleanup_expired_drops_staged_cert() {
    ...
    assert!(
        !cert_manager.has_peer_fingerprint(&device_id),
        "a swept pairing's staged cert must not leak into a later accept (vk #992)"
    );
}

/// vk #992 round 2 (cli-review-panel): the sweep must not wipe the cert
...
async fn test_sweep_preserves_freshly_staged_cert() {
    ...
    assert!(
        cert_manager.has_peer_fingerprint(&device_id),
        "the cert staged with the fresh request must survive the dead stretch's reap (vk #992 round 2)"
    );
}

/// vk #992 round 3 (cli-review-panel, codex): a cert must be staged only
...
async fn test_rejected_request_stages_no_cert() {
    ...
    assert!(
        ...
        "a rejected request's cert must not orphan into a later accept (vk #992 round 3)"
    );
}
...
/// mis-classified the A15's fresh pair request as the accept of our dead one
/// (live failure, 2026-07-30).
#[tokio::test]
...
/// during S21 live validation (2026-07-30) hit the expired request instead
/// of the phone's fresh one.
#[tokio::test]
async fn test_get_verification_key_returns_none_when_expired() {
    ...
    // incoming entry sent an unrequested pair=true onto the wire during S21
    // live validation (pair_device's accept branch fired on it, 2026-07-30).
