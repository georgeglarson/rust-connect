...
impl PluginRegistry {
    ...
    pub async fn register(&self, plugin: Arc<dyn Plugin>) {
...

        {
            ...
            // types the replacement doesn't declare (cubic, PR #7).
    ...
    /// only stable order — and was called out in panel round 2 (kimi-k3).
    /// Index names that no longer resolve (unregister's two-lock window)
    /// are skipped, same tolerance as `get_all_by_capability` (cubic, PR #7).
    ...
    pub async fn handle_packet(
        ...
        // and pays one packet clone per plugin for clarity (panel R4:
        // the router's last-handler clone optimization is not worth
        // duplicating here).
