...
- **Files ≤ 500 lines** (production code, excluding tests) is the target, not an enforced gate. This is the natural result of SRP: a file past it usually has more than one responsibility. Measure production lines only (everything before the file's `#[cfg(test)]`; sibling `*_tests.rs` files do not count). As of 2026-09-06, 16 files exceed it: `plugins/shareinputdevices/mod.rs` 1958, `plugins/mpris/mod.rs` 1248, `plugins/clipboard.rs` 1036, `protocol/connection/mod.rs` 1029, `protocol/pairing/mod.rs` 1021, `protocol/crypto.rs` 1011, `plugins/shareinputdevices/portal.rs` 981, `plugins/mousepad.rs` 966, `plugins/mpris/zbus_backend.rs` 800, `plugins/share.rs` 797, `cli/mod.rs` 773, `plugins/systemvolume/backend.rs` 731, `api/handlers/device.rs` 616, `protocol/connection/inbound.rs` 613, `protocol/payload_transfer.rs` 579, `protocol/types.rs` 533. New code must not add to that list, and a change that touches one of them should split before it grows.
