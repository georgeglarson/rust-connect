...
## 2026-08-02 — Galaxy A15 5G (Android, KDE Connect app `org.kde.kdeconnect_tp`)

Build under test: post-hardening `main` (rcgen certificate stack, SHA-512
signing, no SANs; API on 127.0.0.1; all PR #16 hardening). Host: Linux,
daemon as systemd user service. Networks exercised: Wi-Fi LAN and USB
tether (rndis) — the phone used both during the session.

### Upgrade continuity

- The A15 was paired to the pre-hardening daemon (openssl stack). After
