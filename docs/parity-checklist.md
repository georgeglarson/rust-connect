...
Audit date: 2026-08-04. Revised 2026-08-04: gaps 1/2/4/5 fixed in `db045f8`
...
| Steady-state packet size cap + behavior on oversize | 32 MiB, discard buffer, **continue** (`landevicelink.cpp:19,98-101`) | 32 MiB, skip line, **continue** (M `LanLink.java#46,85-88`) | **CONFORMANT** — 32 MiB cap, oversize line consumed and skipped, link kept alive (fixed `db045f8`) | `src/protocol/connection/mod.rs:59,404-433` (`read_steady_line`) |
| Empty lines skipped | Yes (unserialize-false skip, `networkpacket.cpp:65-73`) | Yes (M `LanLink.java#89-91`) | **CONFORMANT** — blank lines ignored (fixed `db045f8`) | `src/protocol/connection/mod.rs:437-447` |
...
| `id` field type tolerance | Never read — total tolerance (`networkpacket.cpp:46`) | Never read (M `NetworkPacket.kt#295-303`) | **CONFORMANT** — lenient string/number deserializer (fixed `db045f8`) | `src/protocol/types.rs:82-99` |
...
> Fixed since the initial audit (2026-08-04, `db045f8`): the audit's gaps 1
...
> still-open gaps (the audit's 3 and 6-13; vk #994, vk #997, vk #998).
