...
| mDNS resolve behavior | Send UDP identity to resolved address; v8 dial-direct is their TODO (`mdnshdiscovery.cpp:31-36`) | Same (M `MdnsDiscovery.kt` `onServiceResolved` + TODO) | **CONFORMANT** — send our identity to the resolved address, the peer dials us. The v8 dial-direct path (2026-08 → 2026-09) was retired by vk #1101: kdeconnectd can announce SRV port 0 (`lanlinkprovider.cpp:54,139`), so every resolve-triggered dial failed and only the reverse-connection fallback connected it. DIVERGENCE carried: IPv4 only on this leg — the TCP listener is IPv4-only, so an IPv6-only resolve is skipped rather than unicast to (dual-stack listener is the follow-up) | `src/services/service_manager.rs` (`on_mdns_device_resolved`), `src/protocol/udp_unicast.rs` |
