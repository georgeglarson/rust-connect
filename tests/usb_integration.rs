...
///
/// Verified live 2026-07-29 vs Galaxy A15 (rustls stack, both roles):
...
async fn usb_full_protocol_handshake() {
    ...
    // an unpaired device with pair=false (observed live vs A15 2026-07-29).
...
async fn usb_send_file_to_android() {
    ...
    // request (observed live vs A15), so a fresh pair=true is not the only
